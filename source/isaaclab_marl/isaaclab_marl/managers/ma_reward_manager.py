...
class MultiAgentRewardManager(RewardManager):
    def __init__(self, cfg: object, env: ManagerBasedRLEnv):
        ...
        for term_name in self._term_names:
            self._episode_sums[term_name] = torch.zeros(self.num_envs * self.num_agents_per_env, dtype=torch.float, device=self.device)
        ...
        self._step_reward = torch.zeros((self.num_envs * self.num_agents_per_env, len(self._term_names)), dtype=torch.float, device=self.device)
