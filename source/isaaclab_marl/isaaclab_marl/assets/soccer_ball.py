...
class SoccerBall(EnvCoreData):
    ...
    def check_ball_location(self):
        ...
        self.ball_reset = torch.logical_or(
            torch.logical_or(
                self.base_pos_w[:, 0] > self.env_data.field_length / 2,
                self.base_pos_w[:, 0] < -self.env_data.field_length / 2,
            ),
        ...
        self.reset_ball_at_border(self.ball_reset)

    def get_goal_status(self):
        return self.goal_status

    def reset_ball_at_border(self, env_ids):
        if len(env_ids.nonzero(as_tuple=False)) == 0:
            return
        ball_reset_tolerance = 0.005
        root_state = self.asset.data.root_state_w[env_ids]
        # self.get_default_root_state(env_ids)
        root_state[:, :2] = self.env_data.env_origins[env_ids, :2]
